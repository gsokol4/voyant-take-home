...
export default function Search({ query, setQuery, setItems }) {
  ...
  function filter(size) {
    setItems([
      ...crudProcesses
        .searchAll(query)
        .filter((object) => object.size === size),
    ]);
  }

  function sortAToZ(query) {
    setItems([
      ...crudProcesses.searchAll(query).sort((a, b) => {
    ...
    <StyledSearch>
      ...
      <div className="search__util-buttons">
        ...
        <button
          className="search__button"
          onClick={() => random()}
          aria-label="random dog generator"
        >
          Random Pupper
        </button>
        <button
        ...
        <button
          className="search__button"
          onClick={() => sortAToZ(query)}
          aria-label="sort names A-Z"
        >
          sort: By Name A-Z
        </button>
      </div>
